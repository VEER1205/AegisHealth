...
/* ─── Design Tokens ──────────────────────────────────────────── */
const T = {
  red:"#e11d48", redDk:"#be123c", pink:"#f472b6", rose:"#f43f5e",
  white:"#ffffff", bg:"#fdf4f7", border:"#fce7f3", borderDk:"#fbcfe8",
  text:"#1a0a0e", textMd:"#6b2737", textXs:"#c07088", pinkLt:"#fdf2f8",
  sidebar:"#fff7f9",
};
...
function DesktopSidebar({ screen, setScreen, navItems }) {
  return (
    <div style={{width:240,flexShrink:0,display:"flex",flexDirection:"column",background:"#fff7f9",borderRight:`1px solid ${T.border}`,height:"100vh",position:"sticky",top:0,overflow:"hidden"}}>
      {/* Logo */}
...
Until ready, ask one focused follow-up question. Be warm, concise, never diagnose.`;
  ...
  if (isDesktop) {
    return (
      <div style={{display:"flex",width:"100%",height:"100vh",background:T.white,overflow:"hidden",fontFamily:"'DM Sans', sans-serif"}}>
        ...
        <div style={{flex:1,overflowY:"auto",height:"100vh",background:T.white,paddingBottom:40}}>
          {renderScreen()}
        </div>
      </div>
    );
  ...
  /* ── MOBILE LAYOUT ── */
  return (
    <div style={{width:"100%",minHeight:"100svh",display:"flex",justifyContent:"center",alignItems:"flex-start",background:"#f3e6eb",fontFamily:"'DM Sans', sans-serif"}}>
      <EmergencyModal redFlag={redFlag} onDismiss={()=>setRedFlag(null)} />
      <div style={{width:"100%",maxWidth:480,minHeight:"100svh",background:T.white,display:"flex",flexDirection:"column",position:"relative",overflow:"hidden"}}>
        {/* Mobile Header */}
              ...
              <div style={{width:155,height:155,position:"relative"}}>
                {[{top:-1,left:-1,bt:2.5,bl:2.5},{top:-1,right:-1,bt:2.5,br:2.5},{bottom:-1,left:-1,bb:2.5,bl:2.5},{bottom:-1,right:-1,bb:2.5,br:2.5}].map((p,i)=>(
                  <div key={i} style={{position:"absolute",width:20,height:20,
                    ...
                    top:p.top,bottom:p.bottom,left:p.left,right:p.right}}></div>
                ))}
        ...
        {reminders.map((r,idx)=>{
          ...
          return (
            <div key={idx} className="fadeUp" style={{position:"relative",paddingLeft:50,marginBottom:14,animationDelay:`${idx*.07}s`}}>
              <div style={{position:"absolute",left:11,top:16,width:18,height:18,borderRadius:"50%",
